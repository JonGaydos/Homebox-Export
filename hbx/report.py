"""Insurance-grade PDF inventory report, shared by the CLI and GUI."""
import io
...
from hbx.client import format_asset_id
...
def loc_name(item) -> str:
    parent = item.get("parent")
    return parent.get("name", "") if isinstance(parent, dict) else ""


def _purchase_date(item):
    return item.get("purchaseDate") or item.get("purchaseTime")


def _sold_date(item):
    return item.get("soldDate") or item.get("soldTime")
...
class InventoryReport(FPDF):
    ...
    def footer(self):
        # fpdf closes a page only when the next one starts, so the cover flag
        # is already cleared by then; page 1 is always the cover.
    ...
    def add_summary(self, items: list):
        ...
        for idx, item in enumerate(items):
            ...
            self.cell(cols[2], 6, f" {self._t(loc_name(item))[:20]}", fill=True)
            self.cell(cols[3], 6, f"{fmt_price(item.get('purchasePrice'))} ", fill=True, align="R")
    ...
    def add_item(self, item: dict, client, maintenance: list = None):
        self.add_page()
        self.item_count += 1
        name = self._t(item.get("name", "Unknown"))
        asset_id = format_asset_id(item.get("assetId", ""))
        ...
        purchase = [
            ...
            ("Purchase Date",  fmt_date(_purchase_date(item))),
            ("Purchase Price", fmt_price(pp) if pp else ""),
        ]
        ...
        warranty = [
            ...
            ("Expires",           fmt_date(item.get("warrantyExpires"))),
        ...
        if entries:
            ...
            for idx, entry in enumerate(entries):
                ...
                self.cell(mc[2], 5, f" {fmt_date(entry.get('scheduledDate'))}", fill=True)
                self.cell(mc[3], 5, f" {fmt_date(entry.get('completedDate'))}", fill=True)
                cost = float(entry.get("cost") or 0)
                self.cell(mc[4], 5, fmt_price(cost) if cost else " --", fill=True, align="R")
                self.ln()
        ...
        sold = [
            ...
            ("Sold Date",  fmt_date(_sold_date(item))),
            ("Sold Price", fmt_price(sold_price) if sold_price else ""),
